...
for (sim in 1:nSim) { ## loop over independent simulations
  ...
  pdf(file = paste(currResPath, "/sim", sim, "_R", r0.true, "_theta.pdf", sep = ""), width = 7, height = 7)
  par(mar = c(5,5,2,2))
  ...
  ## smoothing moments:
  pdf(file = paste(currResPath, "/sim", sim, "_R", r0.true, "_sMoments.pdf", sep = ""), width = 7*ceiling(nCL/ceiling(sqrt(nCL))), height = 3*ceiling(sqrt(nCL)))
  ...
  pdf(file = paste(currResPath, "/sim", sim, "_R", r0.true, "average_sMoments.pdf", sep = ""), width = 7, height = 5)
...
save.image(paste(currResPath, "/output_R", r0.true, ".Rdata", sep = ""))
