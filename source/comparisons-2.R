...
if(!"branchCorr" %in% rownames(inst.pkgs)){
  install_github("jasonxu90/branchCorr")
}

lapply(c(l.pkgs, "branchCorr"), function(pkg){library(pkg,character.only=TRUE)})
...
resPath <- paste("./results_c2", paste(rtf, collapse = "-"), sep = "_")
...
th.true["HSC->P1"] <- (th.true["P1->A"] + th.true["P1->B"] + th.true["P1->C"])/1
th.true["HSC->P2"] <- (th.true["P2->D"] + th.true["P2->E"])/1
...
allParamsBranchCorr <- matrix(data = NA, nrow = length(phi.true) - 2, ncol = nSim)
rownames(allParamsBranchCorr) <- head(names(phi.true),-2)
...
for (sim in 1:nSim) { ## loop over independent simulations
  ...
  synthData <- t(apply(Y0[,head(LETTERS,5),], 3, function(y){c(y)}))
  obsTimes <- as.numeric(rownames(Y0))
  totalPopulations <- colSums(synthData)
  nTot <- matrix(totalPopulations, ncol = length(obsTimes), byrow = T)

  th.0 <- th.true
  death.par <- th.true[c(11:15)]
  th.0[setdiff(names(th.0), names(death.par))] <- 1

  numInits <- 300

  sampSize <- ceiling(max(colSums(synthData)))
  sampledData <- hyperGeoSample(synthData, sampSize)
  cat("Fit branchCorr...")
  res.branch <- optimizeSamplingCorrNLMINB(data = sampledData,
                                           numInits = numInits,
                                           initMean = th.0,
                                           obsTimes = obsTimes,
                                           nSample = rep(sampSize, ncol(Y0) - 3),
                                           nTotal = nTot,
                                           trueDeaths = death.par,
                                           progStructure = progStructure)
  cat("done\n")

  avg.par <- apply(matrix(res.branch, ncol = length(th.true) + 3)[,1:length(rcts), drop = FALSE], 2, mean)

  pdf(file = paste(currResPath, "/branchCorr-sim", sim, "_f", f_NA, "_r", r0.true, "_t", ntps, "_theta.pdf", sep = ""), width = 7, height = 7)
  plot(th.true, avg.par,
       xlim = range(c(th.true, avg.par)),
       ylim = range(c(th.true, avg.par)),
       cex.axis = 2, cex.lab = 2, pch = 21, cex = 5,
       col = c(rep("black", length(rcts) - 5),
               rep("blue", 5)))
  lines(x = range(c(th.true, head(res.fit$fit$par, -2))),
        y = range(c(th.true, head(res.fit$fit$par, -2))), lwd = 3, lty = 2, col = "red")
  text(x = th.true,
       y = avg.par,
  ...
  allParamsBranchCorr[,sim] <- avg.par
...
all.res <- data.frame(variable = rep(rownames(head(allParamsKaren,-2)), times = 3*nSim),
                      value = as.numeric(c(c(head(allParamsKaren,-2)/th.true),
                                           c(head(allParamsKaren_noCnstr,-2)/th.true),
                                           c(allParamsBranchCorr/th.true))),
                      method = c(rep("Karen", each = length(th.true)*nSim),
                                 rep("Karen_noCnstr", each = length(th.true)*nSim),
                                 rep("branchCorr", each = length(th.true)*nSim)))

all.res <- subset(all.res, method != "branchCorr"  | !(variable %in% names(tail(th.true,5))))
