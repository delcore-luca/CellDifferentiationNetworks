...
f_NA <- as.numeric(commandArgs(trailingOnly = TRUE))
...
for (sim in 1:nSim) { ## loop over independent simulations
  ...
  pdf(file = paste(currResPath, "/sim", sim, "_f", f_NA, "_theta.pdf", sep = ""), width = 7, height = 7)
  par(mar = c(5,5,2,2))
  ...
  ## smoothing moments:
  pdf(file = paste(currResPath, "/sim", sim, "_f", f_NA, "_sMoments.pdf", sep = ""), width = 7*ceiling(nCL/ceiling(sqrt(nCL))), height = 3*ceiling(sqrt(nCL)))
  ...
  pdf(file = paste(currResPath, "/sim", sim, "_f", f_NA, "average_sMoments.pdf", sep = ""), width = 7, height = 5)
...
save.image(paste(currResPath, "/output_f", f_NA, ".Rdata", sep = ""))
