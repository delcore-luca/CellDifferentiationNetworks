...
vectorID_mod <- as.vector(unlist(strsplit(commandArgs(trailingOnly = TRUE), split = "-", fixed = TRUE)))
# vectorID_mod <- as.vector(unlist(strsplit("PGK-2", split = "-", fixed = TRUE)))
vectorID <- vectorID_mod[1]
...
nMod <- as.numeric(vectorID_mod[2])

############### MODEL FITTING #####################
Y0 <- Y
rm(Y)

cat(paste("Vector ID: ", vectorID, "\t Model n. ", nMod, "\n", sep = ""))
...
save.image(paste(currResPath, "/vectorID_", vectorID, "_Model", nMod, "_output.Rdata", sep = ""))
...
nCL <- dim(Y0)[3]
pdf(file = paste(currResPath, "/vectorID_", vectorID, "_Model", nMod, "_sMoments.pdf", sep = ""), width = 7*ceiling(nCL/ceiling(sqrt(nCL))), height = 3*ceiling(sqrt(nCL)))
...
pdf(file = paste(currResPath, "/vectorID_", vectorID, "_Model", nMod, "average_sMoments.pdf", sep = ""), width = 7, height = 5)
...
pdf(file = paste(currResPath, "/vectorID_",vectorID, "_Model", nMod, "diffNet.pdf", sep = ""), width = 5, height = 5)
...
pdf(file = paste(currResPath, "/vectorID_",vectorID, "data.pdf", sep = ""), width = 7, height = 5)
par(mar = c(5,5,2,2), mfrow = c(1,1))
